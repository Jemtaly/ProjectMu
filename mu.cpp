...
struct WavHead {
	...
	WavHead(uint32_t const &ds):
		data_size(ds), fmt_size(sizeof fmt), riff_size(sizeof format + sizeof fmt_ID + sizeof fmt_size + sizeof fmt + sizeof data_ID + sizeof data_size + ds) {}
};
...
class Passage {
	...
	struct Note {
		...
		Rational value;
		Note(int const &n, int const &a, Rational const &l):
			solfa(n), accidental(a), octave(0), value(l) {}
	};
	vector<Note> notes;
	struct Warn {
		int measure;
		Rational mval;
		Warn(int const &m, Rational const &v):
			measure(m), mval(v) {}
	};
	vector<Warn> warns;
public:
	Passage(string const &md, int const (&mt)[2], int const &tm, istream &input):
		mode(md), metre{mt[0], mt[1]}, bpm(tm) {
		int persist[7] = {};
